...
if diag_.UnboundedContinuousLikelihood
    % Note on wording: this is a statement about TIES, not about how xmin
    % was chosen. The function cannot tell a protocol threshold that
    % happens to equal min(eventseries) from one computed as
    % min(eventseries), so it must not accuse the caller of either.
    rec = sprintf(['%d observation(s) sit exactly at xmin=%.6g, so the ' ...
        ...
        'divergence are excluded from selection. Ties at xmin are normal ' ...
        'when a correctly specified truncation threshold lands on a grid ' ...
        'value of discrete durations, so the remedy is the discrete model, ' ...
        'not a different xmin: use DistributionType="discrete" ' ...
        '(SamplingInterval=%.6g).'], ...
...
if diag_.XminEqualsDataMin && options.Verbose
    % Only the coincidence is reported. Whether it is benign depends on
    % provenance, which is not observable from the inputs.
    fprintf(['FitHyperexponentialMLE: xmin (%.6g) equals the smallest ' ...
        'retained duration. If that is your protocol threshold, passed as ' ...
        'a fixed value, this is expected and needs no action. If xmin was ' ...
        'computed as min(eventseries), note that it then tracks the sample ' ...
        'and can drift between datasets, changing what the fit conditions ' ...
        'on.\n'], xmin);
end
...
s = struct('K', NaN, 'k', NaN, 'n', NaN, 'WeightsObserved', [], ...
    ...
    'PointwiseLogLik', [], 'SurvivalHandle', [], ...
    'BackTransformExponent', NaN, ...
...
% Truncated survival handle, P(T > t | T >= xmin), built from the FINAL
% (sorted) parameters. In q coordinates this is simply a q-weighted sum of
% per-component survivals, with no normalizer -- the same simplification
% that removed log S(xmin) from the likelihood.
qf = out.WeightsObserved(:).';
rf = out.Rates(:).';
if isDiscrete
    dtf = options.SamplingInterval;
    out.SurvivalHandle = @(t) min(sum(qf .* ...
        exp(-rf .* max(round(t(:)/dtf) - n_min + 1, 0) * dtf), 2), 1);
else
    out.SurvivalHandle = @(t) min(sum(qf .* ...
        exp(-rf .* max(t(:) - xmin, 0)), 2), 1);
end
